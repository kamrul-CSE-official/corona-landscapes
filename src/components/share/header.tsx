...
const Header = () => {
  ...
  return (
    <div
      className={`${
        !shrink ? "fixed w-full z-50 transition-all duration-700 bg-transparent py-6" : "fixed w-full z-50 transition-all duration-700 bg-white py-2 shadow-sm"
      }`}
    >
      <div className={`${!shrink ? "max-w-7xl mx-auto px-8 lg:px-16 flex justify-between items-center": "max-w-7xl mx-auto px-8 lg:px-16 flex justify-between items-center"}`}>
        {/* Logo */}
            ...
            navItems.map((item,i) => (
              <Link
                ...
                className={`transition-all duration-500 ${
                  !shrink
                    ? "text-[11px] font-bold tracking-[0.3em] uppercase transition-all hover:opacity-100 text-white opacity-60"
                    : "text-[11px] font-bold tracking-[0.3em] uppercase transition-all hover:opacity-100 text-[#1a2e30] opacity-50"
                }`}
              >
                {item.name}
              </Link>
